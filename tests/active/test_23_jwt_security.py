...
class TestJwtStorage:
    def test_jwt_not_in_localStorage(self, base_html):
        """Check if JWT is stored in browser localStorage (vulnerable to XSS)."""
        insecure_patterns = [
            re.compile(r"localStorage\.setItem.*jwt|localStorage\.setItem.*token"),
            re.compile(r"localStorage\[.*jwt\s*\]|localStorage\[.*token\s*\]"),
        ]
        for pattern in insecure_patterns:
            if pattern.search(base_html):
                pytest.fail("JWT token stored in localStorage — vulnerable to XSS")
