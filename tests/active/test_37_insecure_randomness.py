...
class TestTokenMathRandomness:
    def test_math_random_detection_in_source(self, base_html):
        """Passive check: look for Math.random() in source code."""
        if "Math.random()" in base_html or "math.random" in base_html.lower():
            pytest.skip(
                "Math.random() detected in source code — may indicate weak randomness "
                "if used for security tokens. Verify by analyzing token patterns."
            )
